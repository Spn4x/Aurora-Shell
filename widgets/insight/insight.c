...
// DELETED: The hard-coded global colors are gone.
...
// NEW: Helper function to get named colors from the widget's CSS context.
...
// MODIFIED: This function now gets its colors from CSS.
static void draw_custom_app_bar(GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer user_data) {
    ...
    // Fetch colors dynamically from CSS
    GdkRGBA accent_color, trough_color;
    get_css_color(GTK_WIDGET(area), "accent", &accent_color);
    // For the trough, we use the foreground color with low opacity, like in the CSS
...
static gboolean update_data_and_ui(gpointer user_data) {
    ...
    if (is_current_week) {
        ...
        GDate *ref_copy = g_date_copy(data->reference_date);
        g_date_add_days(ref_copy, 7);
        gboolean is_last_week = (g_date_get_iso8601_week_of_year(data->today_date) == g_date_get_iso8601_week_of_year(ref_copy) && g_date_get_year(data->today_date) == g_date_get_year(ref_copy));
        g_date_free(ref_copy);
        if (is_last_week) {
            gtk_label_set_text(GTK_LABEL(data->week_title_label), "Last Week");
        } else {
            char range_str[64];
            GDate *start = g_date_copy(data->reference_date);
            g_date_set_day(start, g_date_get_day(start) - (g_date_get_weekday(start) % 7));
            GDate *end = g_date_copy(start);
            g_date_add_days(end, 6);
            char start_str[32], end_str[32];
            g_date_strftime(start_str, sizeof(start_str), "%b %d", start);
            g_date_strftime(end_str, sizeof(end_str), "%b %d", end);
            snprintf(range_str, sizeof(range_str), "%s - %s", start_str, end_str);
            gtk_label_set_text(GTK_LABEL(data->week_title_label), range_str);
            g_date_free(start); g_date_free(end);
        }
    }
    gtk_widget_set_sensitive(data->next_week_button, !is_current_week);

    if (g_date_compare(data->selected_date, data->today_date) == 0) {
        gtk_label_set_text(GTK_LABEL(data->selected_day_title_label), "Today");
    } else {
        GDate *yesterday = g_date_copy(data->today_date);
        g_date_subtract_days(yesterday, 1);
        if (g_date_compare(data->selected_date, yesterday) == 0) {
            gtk_label_set_text(GTK_LABEL(data->selected_day_title_label), "Yesterday");
        } else {
            char day_name[32];
            g_date_strftime(day_name, sizeof(day_name), "%A", data->selected_date);
            gtk_label_set_text(GTK_LABEL(data->selected_day_title_label), day_name);
        }
    ...
    if (sqlite3_prepare_v2(db, sql_top_apps, -1, &stmt, 0) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, selected_date_str, -1, SQLITE_STATIC);
        while (sqlite3_step(stmt) == SQLITE_ROW && app_count < 5) {
            strncpy(top_apps[app_count].app_class, (const char*)sqlite3_column_text(stmt, 0), 255);
            top_apps[app_count].seconds = sqlite3_column_int(stmt, 1);
            app_count++;
        }
    }
    sqlite3_finalize(stmt);
    ...
    if (app_count == 0) {
        GtkWidget *placeholder = gtk_label_new("No activity recorded");
        ...
        gtk_box_append(GTK_BOX(data->top_apps_box), placeholder);
    }

    for (int i = 0; i < app_count; i++) {
        GtkWidget *row_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 3);
        GtkWidget *labels_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
        char* pretty_name = prettify_app_name(top_apps[i].app_class); GtkWidget *name_label = gtk_label_new(pretty_name); g_free(pretty_name);
        gtk_widget_set_hexpand(name_label, TRUE); gtk_widget_set_halign(name_label, GTK_ALIGN_START);
        char app_time_str[64];
        long seconds = top_apps[i].seconds;
        ...
        gtk_box_append(GTK_BOX(labels_hbox), name_label); gtk_box_append(GTK_BOX(labels_hbox), time_label);
        GtkWidget *bar = gtk_drawing_area_new();
        gtk_widget_set_size_request(bar, -1, 8);
        CustomBarData *bar_data = g_new(CustomBarData, 1);
        bar_data->fraction = (double)top_apps[i].seconds / max_seconds;
        gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(bar), draw_custom_app_bar, bar_data, g_free);
        gtk_box_append(GTK_BOX(row_vbox), labels_hbox);
        gtk_box_append(GTK_BOX(row_vbox), bar);
        gtk_box_append(GTK_BOX(data->top_apps_box), row_vbox);
    }
    sqlite3_close(db);
...
// --- UI Callbacks ---
static void on_prev_week_clicked(GtkButton *button, gpointer user_data) {
    (void)button;
    AppData *data = (AppData *)user_data;
    g_date_subtract_days(data->reference_date, 7);
    g_date_free(data->selected_date);
    data->selected_date = g_date_copy(data->reference_date);
    update_data_and_ui(data);
}
static void on_next_week_clicked(GtkButton *button, gpointer user_data) {
    (void)button;
    AppData *data = (AppData *)user_data;
    g_date_add_days(data->reference_date, 7);
    g_date_free(data->selected_date);
    data->selected_date = g_date_copy(data->reference_date);
    update_data_and_ui(data);
}
static void on_chart_clicked(GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (x >= x_pos && x <= x_pos + bar_width) { clicked_index = i; break; }
    }
    if (clicked_index != -1) {
        GDate *week_start = g_date_copy(data->reference_date);
        g_date_set_day(week_start, g_date_get_day(week_start) - (g_date_get_weekday(week_start) % 7));
...
static GtkWidget* create_stat_box(const char* title, GtkWidget **title_label_ptr, GtkWidget **time_label_ptr) {
    GtkWidget *vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
    *title_label_ptr = gtk_label_new(title);
    gtk_widget_set_halign(*title_label_ptr, GTK_ALIGN_START);
    gtk_box_append(GTK_BOX(vbox), *title_label_ptr);
    *time_label_ptr = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(*time_label_ptr), "<span size='x-large' weight='bold'>0h 0m</span>");
    gtk_widget_set_halign(*time_label_ptr, GTK_ALIGN_START);
    gtk_box_append(GTK_BOX(vbox), *time_label_ptr);
    return vbox;
}
static void on_widget_destroy(GtkWidget *widget, gpointer user_data) {
    ...
    g_date_free(data->today_date);
    g_date_free(data->selected_date);
    g_date_free(data->reference_date);
}
...
GtkWidget* create_widget(const char* config_string) {
    ...
    app_data->timer_id = g_timeout_add_seconds(60, (GSourceFunc)update_data_and_ui, app_data);
    return root_vbox;
}
