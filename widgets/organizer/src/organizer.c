...
static void on_clear_all_clicked(GtkButton *button, gpointer user_data) {
    ...
    while (child) {
        ...
        if (GTK_IS_BUTTON(close_button)) { on_close_clicked(GTK_BUTTON(close_button), child); }
        child = next_sibling;
    }
}
...
static void on_get_initial_dnd_state(GObject *source_object, GAsyncResult *res, gpointer user_data) {
    ...
    if(error) { g_warning("Organizer: Could not get initial DND state: %s", error->message); return; }
    gboolean is_active;
